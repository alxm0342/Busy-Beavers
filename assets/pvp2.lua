...
function scene:createScene( event )
	...
	p1ScoreDisplay = display.newText(twigs.getScore(),45,465,native.systemFontBold,12)
	p1ScoreDisplay.text = twigs.getP1Score()
	p2ScoreText = display.newText("P2:", 80, 465, native.systemFontBold, 12)
	p2ScoreDisplay = display.newText(twigs.getScore(),115,465,native.systemFontBold,12)
...
function scene:enterScene( event )
	pvp2Music = audio.loadStream("media/music/level1.mp3")
	if (game_options.getMusic() == 'on') then
		pvp2MusicChannel = audio.play(pvp2Music, {loops=-1})
	end
	local p2Start = display.newImage("media/p2Start.png")
	...
	countdown = timer.performWithDelay(1000,
		function()
			...
			p2ScoreDisplay.text = twigs.getScore()
			if (lvlTimer.getTime()-90 == 0)  then
				lvlTimer.resetTime()
				twigs.saveP2Score()
				twigs.resetScore()
				storyboard.gotoScene("assets.pvp2_complete")
			end
		end, 0)
...
function scene:exitScene( event )
	audio.stop(pvp2MusicChannel)
	pvp2MusicChannel = nil
	pvp2Music = nil
