	...
	countdown = timer.performWithDelay(1000,
		function()
			...
			if (lvlTimer.getTime() == 0 or twigs.getCollected() == 8) then
				storyboard.gotoScene("assets.level1_complete")
			end
		end, 0)
	...
	if event.phase == "began" then
		...
		if paused == true then
			...
			elseif event.x > 100 and event.x < 220 and event.y > 130 and event.y < 160 and menu.isVisible == true then
				gameSettings.level1 = true
				gameSettings.level2 = false
				...
				gameState.saveTable(gameSettings, "gamesettings.json", system.DocumentsDirectory)
				print('game saved!!')
